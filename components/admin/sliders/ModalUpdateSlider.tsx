...
const ModalUpdateSlider: React.FC<ModalUpdateSliderProps> = ({
  handleClose,
  open,
  current,
}) => {
  ...
  useEffect(() => {
    current?.url && setValue("url", current?.url);
    if (current?.image) {
      ...
      <div className="w-[90%] md:w-[500px] p-4 rounded-lg bg-white fixed z-[70] top-[50%] translate-y-[-50%] translate-x-[-50%] left-[50%] ">
        ...
        <div className="mt-4 space-y-2">
          ...
          <div className="space-y-2">
            <label>URL: {`không bắt buộc`}</label>
            <TextField
              control={control}
              error={errors}
              name="url"
...
export default ModalUpdateSlider;
