  ...
  return (
    <div className="mt-5">
      {data?.map((item) => (
        ...
        >
          <div className="p-2 font-bold">{dataStatus.find(element => element.value == item.status)?.text || "Không có"}</div>
          ...
          <div className="text-right overflow-auto mr-5">
            <div className="font-bold text-base py-1">Tổng tiền: {formatPrices(item.total)}</div>
